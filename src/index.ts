...
// Type-only: resolves the required ctx.skills service declaration.
import type {} from '@deepseek-ai/dsh-skill'
import { SKILL_CONTENT, SKILL_DESCRIPTION, SKILL_WHEN_TO_USE } from './skill.ts'
import {
  findConfoundedPairs, formatConfigValue, formatStat, marginalEffect, planSweep, rankTrials,
} from './sweep.ts'
import type { MetricDirection, SearchSpace, SweepCampaign, SweepStrategy, SweepTrial, TrialStatus } from './types.ts'
...
const RECORDABLE_STATUSES: readonly TrialStatus[] = ['running', 'ok', 'failed', 'pruned']
...
function parseSearchSpace(value: JsonValue): SearchSpace {
  ...
  for (const [parameter, spec] of Object.entries(value)) {
    ...
    space[parameter] = spec as unknown as SearchSpace[string]
  }
 ...
 */
function parseRecord(value: JsonValue | undefined, label: string): Record<string, JsonValue> {
 ...
 */
function parseMetrics(value: JsonValue | undefined): Record<string, number> {
...
/** One opened ledger per calling agent, held for this plugin fiber's lifetime. */
type Ledger = Map<string, SweepCampaign>

/** Read one campaign for a calling agent, failing loud when the id is unknown. */
function requireCampaign(ledgers: Map<string, Ledger>, owner: string, id: string): SweepCampaign {
  const campaign = ledgers.get(owner)?.get(id)
  if (campaign === undefined) {
    const known = [...(ledgers.get(owner)?.keys() ?? [])]
    throw new Error(
      `unknown campaign ${JSON.stringify(id)} for this session; known: ${known.length === 0 ? 'none' : known.join(', ')}`,
    )
  }
  return campaign
}
...
function renderReport(campaign: SweepCampaign, metric: string, direction: MetricDirection, topK: number): string {
  const counts: Record<TrialStatus, number> = { running: 0, ok: 0, failed: 0, pruned: 0 }
  ...
  out.push(
    `Trials: ${counts.ok} ok, ${counts.failed} failed, ${counts.pruned} pruned, ${counts.running} running,`
  ...
  const delta = (value: number, reference: number | undefined): string => {
  ...
  shown.forEach((entry, index) => {
    ...
    out.push(
      `| ${index + 1} | ${entry.trial.id} | ${formatStat(entry.value)}`
  ...
  const best = shown[0]
  if (best === undefined) return out.join('\n')
  ...
  out.push(
    `- sota target: ${sotaValue === undefined || campaign.sota === null
  ...
  if (runnerUp !== undefined) {
    ...
    if (third !== undefined) out.push(`- gap between rank 1 and rank 3: ${formatStat(Math.abs(best.value - third.value))}`)
  ...
  const distinctValues = (parameter: string): number =>
    new Set(scored.map(entry => JSON.stringify(entry.trial.config[parameter] ?? null))).size
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      'Open a tuning ledger and expand a search space into a bounded, de-confounded trial matrix. '
    ...
    parameters: {
      ...
      space: {
        ...
        required: true,
        description:
          'Parameter name to either an explicit value array or a range spec {min, max, steps, log?}. '
    ...
    execute(args, exec) {
      ...
      if (owner === undefined) throw new Error('research_sweep_plan requires an owning agent session')
      const id = args.campaign.trim()
      if (id === '') throw new Error('campaign must be a non-empty id')
      const ledger = ledgers.get(owner.id) ?? new Map<string, SweepCampaign>()
      ...
      const strategy: SweepStrategy = args.strategy ?? 'grid'
      ...
      const trials: SweepTrial[] = plan.trials.map(trial => ({
        id: trial.id,
        config: trial.config,
        status: 'running',
        metrics: null,
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      'Record one finished trial into the tuning ledger, including failed and pruned runs, '
    ...
    execute(args, exec) {
      const owner = exec.agent
      if (owner === undefined) throw new Error('research_sweep_record requires an owning agent session')
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      'Rank a tuning ledger by one metric, show the best configuration, the margin against baseline and '
    ...
    execute(args, exec) {
      const owner = exec.agent
      if (owner === undefined) throw new Error('research_sweep_report requires an owning agent session')
      const campaign = requireCampaign(ledgers, owner.id, args.campaign.trim())
